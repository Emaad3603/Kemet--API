    ...
    public class SearchServices : ISearchInterface
    {
        ...
        public async Task<RagionalSearchDTO> SearchAll(string textA , UserManager<AppUser> userManager)
        {
            try
            {
                ...
                var Activities = await _context.Activities.AsNoTracking().Where(A => A.Name.Contains(text) || A.Location.Address.Contains(text) || A.Category.CategoryName.Contains(text)).ToListAsync();
